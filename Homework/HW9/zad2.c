...
void *linearSearch(const void*, const void*, size_t, size_t, funcPointer_t);
...
int main()
{
    ...
    books[5].price = 20.50;
    qsort(books, COUNT, sizeof(*books), compareFunctions[6]);
    for(int i = 0; i < COUNT; i++)
    {
        printBook(books+i);
    }
    //Book key = {.price = 20.50, .author = "", .title = "", .pages = 0}; --> 1vi nachin
    double key = 20.50; // --> 2ri nachin
    Book* element = linearSearch(&key, books, COUNT, sizeof(*books), comparePrices);
    if(!element)
    {
        printf("Not found!\n");
    }
...
void *linearSearch(const void* key, const void* base, size_t numElements , size_t sizeElement, funcPointer_t compare)
{
    for (int i = 0; i < numElements; i++)
    {
        if(compare(key, base + (i * sizeElement)) == 0)
        {
            return (void *)base + (i * sizeElement);
        }
    }
    return NULL;
}
//compare funkciq izpolzvana za 2ri nachin
int comparePrices(const void *p1, const void *p2)
{
    double price1 = *((double*)p1);
    double price2 = ((Book *)p2)->price;
...
//compare funkciq izpolzavana za 1vi nachin
